...
(import ./lib.nix) {
  ...
  testScript = ''
    ...
    with subtest("Test paperless"):
      ...
      lab.wait_until_succeeds(wait_for_paperless_cmd, timeout=60)

      # Wait for paperless celery working to start
      wait_for_paperless_celery_cmd = 'journalctl --boot --no-pager --quiet --unit docker.service --grep "celery.*ready"'
      lab.wait_until_succeeds(wait_for_paperless_celery_cmd, timeout=60)
    ...
    with subtest("Attempt to run lab backup"):
      ...
      wait_for_paperless_db = 'ls -nd /var/cache/docker-compose-runner/paperless/data/db.sqlite3'
      lab.wait_until_succeeds(wait_for_paperless_db, timeout=60)

      # Check that the permissions are correct
      lab.succeed("ls -nd /var/cache/docker-compose-runner/paperless/data/db.sqlite3 | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")
      ...
      backup.succeed("test -d /tmp/backup-root-lab/docker-compose-runner/paperless/data")

      # Check that known files exist and permissions are correct
      backup.succeed("test -e /tmp/backup-root-lab/docker-compose-runner/paperless/data/db-snapshot-" + labdayofweek + ".sqlite3")
      backup.succeed("ls -nd /tmp/backup-root-lab/docker-compose-runner/paperless/data/db-snapshot-" + labdayofweek + ".sqlite3 | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")
    ...
    with subtest("Test Auto Backup Machines"):
      ...
      # Check volumes are appearing
      lab.succeed("test -d /mnt/pool/data/backup/lab/var/lib/docker-compose-runner/paperless/data")

      # Check that known files exist and permissions are correct
      lab.succeed("test -e /mnt/pool/data/backup/lab/var/lib/docker-compose-runner/paperless/data/db-snapshot-" + labdayofweek + ".sqlite3")
      lab.succeed("ls -nd /mnt/pool/data/backup/lab/var/lib/docker-compose-runner/paperless/data/db-snapshot-" + labdayofweek + ".sqlite3 | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")
