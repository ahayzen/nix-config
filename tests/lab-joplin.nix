...
(import ./lib.nix) {
  ...
  testScript = ''
    ...
    with subtest("Test joplin"):
      ...
      wait_for_joplin_cmd = 'journalctl --boot --no-pager --quiet --unit docker.service --grep "App: Public base URL: http://localhost:22300"'
      lab.wait_until_succeeds(wait_for_joplin_cmd, timeout=60)

      # Test login page
      output = vps.succeed("curl --insecure --location --silent joplin.hayzen.uk")
