...
(import ./lib.nix) {
  ...
  nodes = {
    vps = { self, pkgs, ... }: {
      ...
      networking.hosts = {
        "127.0.0.1" = [ "wg-easy.hayzen.uk" "ahayzen.com" ];
      };
  ...
  testScript = ''
    ...
    with subtest("Test wg-easy"):
      # Wait for wg-easy to start
      wait_for_wg_easy_cmd = 'journalctl --boot --no-pager --quiet --unit docker.service --grep "Listening on http://0.0.0.0:80"'
      lab.wait_until_succeeds(wait_for_wg_easy_cmd, timeout=60)

      # Test login page wg-quick
      output = vps.succeed("curl --insecure --location --silent wg-easy.hayzen.uk")
      assert "WireGuard" in output, f"'{output}' does not contain 'WireGuard'"

      # Test login page directly
      output = lab.succeed("curl --insecure --location --silent wg-easy.hayzen.uk")
      assert "WireGuard" in output, f"'{output}' does not contain 'WireGuard'"
    ...
    with subtest("Attempt to run lab backup"):
      ...
      lab.succeed("ls -nd /var/cache/docker-compose-runner/wg-easy/config/wg-easy.db | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")

      # Trigger a snapshot
      labdayofweek = datetime.datetime.now(zoneinfo.ZoneInfo("Europe/London")).strftime('%w')
      lab.succeed("systemctl start wg-easy-db-snapshot.service")
      ...
      backup.succeed("test -d /tmp/backup-root-lab/docker-compose-runner/wg-easy/config")

      # Check that known files exist and permissions are correct
      backup.succeed("test -e /tmp/backup-root-lab/docker-compose-runner/wg-easy/config/wg-easy-snapshot-" + labdayofweek + ".db")
      backup.succeed("ls -nd /tmp/backup-root-lab/docker-compose-runner/wg-easy/config/wg-easy-snapshot-" + labdayofweek + ".db | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")
    ...
    with subtest("Test Auto Backup Machines"):
      ...
      # Check volumes are appearing
      lab.succeed("test -d /mnt/pool/data/backup/lab/var/lib/docker-compose-runner/wg-easy/config")

      # Check that known files exist and permissions are correct
      lab.succeed("test -e /mnt/pool/data/backup/lab/var/lib/docker-compose-runner/wg-easy/config/wg-easy-snapshot-" + labdayofweek + ".db")
      lab.succeed("ls -nd /mnt/pool/data/backup/lab/var/lib/docker-compose-runner/wg-easy/config/wg-easy-snapshot-" + labdayofweek + ".db | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")
