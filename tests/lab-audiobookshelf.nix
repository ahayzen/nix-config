...
(import ./lib.nix) {
  ...
  nodes = {
    vps = { self, pkgs, ... }: {
      ...
      networking.hosts = {
        "127.0.0.1" = [ "audiobookshelf.hayzen.uk" "ahayzen.com" ];
      };
    ...
    lab = { self, lib, pkgs, ... }: {
      ...
      networking.hosts = {
        ...
        "127.0.0.1" = [ "audiobookshelf.hayzen.uk" ];
  ...
  testScript = ''
    ...
    with subtest("Test audiobookshelf"):
      ...
      output = lab.succeed("curl --insecure --location --silent audiobookshelf.hayzen.uk/login")
      assert "Audiobookshelf" in output, f"'{output}' does not contain 'Audiobookshelf'"
    ...
    with subtest("Attempt to run lab backup"):
      ...
      lab.succeed("ls -nd /var/cache/docker-compose-runner/audiobookshelf/config/absdatabase.sqlite | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")
