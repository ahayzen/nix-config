...
(import ./lib.nix) {
  ...
  nodes = {
    vps = { self, pkgs, ... }: {
      ...
      networking.hosts = {
        "127.0.0.1" = [ "nextcloud.hayzen.uk" "ahayzen.com" "yumekasaito.com" ];
      };
    ...
    lab = { self, lib, pkgs, ... }: {
      ...
      networking.hosts = {
        ...
        "192.168.1.3" = [ "nextcloud.hayzen.uk" "ahayzen.com" "yumekasaito.com" ];
      };
  ...
  testScript = ''
    ...
    with subtest("Test nextcloud"):
      ...
      lab.wait_until_succeeds(wait_for_nextcloud_cmd, timeout=60)

      time.sleep(120)
      print(lab.succeed("docker ps --all"))
      print(lab.succeed("df -h"))
      print(lab.succeed("free -h"))
      print(lab.succeed("ps auxf"))
      print(lab.succeed("docker stats --no-stream"))
      print(lab.succeed("ls -la /var/cache/docker-compose-runner/nextcloud/html"))
      print(lab.succeed("docker exec -it -t nextcloud ls -la /usr/src/nextcloud"))
      print(lab.succeed("docker exec -it -t nextcloud ls -la /usr/src/nextcloud/config"))
      print(lab.succeed("docker exec -it -t nextcloud ls -la /var/www/html"))
      wait_for_nextcloud_cmd = 'journalctl --boot --no-pager --quiet --unit docker.service --grep "Initializing finished"'
      lab.wait_until_succeeds(wait_for_nextcloud_cmd, timeout=120)

      # Test login page
      output = vps.succeed("curl --insecure --location --silent nextcloud.hayzen.uk/")
      assert "Create administation account" in output, f"'{output}' does not contain 'Create administration account'"
    ...
    with subtest("Attempt to run lab backup"):
      ...
      # Paperless database can take a while to appear
      wait_for_nextcloud_db = 'ls -nd /var/cache/docker-compose-runner/nextcloud/data/owncloud.db'
      lab.wait_until_succeeds(wait_for_nextcloud_db, timeout=60)

      # Check that the permissions are correct
      lab.succeed("ls -nd /var/cache/docker-compose-runner/nextcloud/data/owncloud.db | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")
      ...
      backup.succeed("test -d /tmp/backup-root-lab/docker-compose-runner/nextcloud/data")

      # Check that known files exist and permissions are correct
      backup.succeed("test -e /tmp/backup-root-lab/docker-compose-runner/nextcloud/data/owncloud-snapshot-" + labdayofweek + ".db")
      backup.succeed("ls -nd /tmp/backup-root-lab/docker-compose-runner/nextcloud/data/owncloud-" + labdayofweek + ".db | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")
    ...
    with subtest("Test Auto Backup Machines"):
      ...
      # Check volumes are appearing
      lab.succeed("test -d /mnt/pool/data/backup/lab/var/lib/docker-compose-runner/nextcloud/data")

      # Check that known files exist and permissions are correct
      lab.succeed("test -e /mnt/pool/data/backup/lab/var/lib/docker-compose-runner/nextcloud/data/owncloud-snapshot-" + labdayofweek + ".db")
      lab.succeed("ls -nd /mnt/pool/data/backup/lab/var/lib/docker-compose-runner/nextcloud/data/owncloud-snapshot-" + labdayofweek + ".db | awk 'NR==1 {if ($3 == 2000) {exit 0} else {exit 1}}'")
